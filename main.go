...
func main() {
	...
	for {
		...
		if strings.HasPrefix(line, ":bv") {
			args := strings.TrimSpace(strings.TrimPrefix(line, ":bv"))
			report, err := bvCommand(args)
			if err != nil {
				fmt.Println(err)
				continue
			}
		...
		if strings.HasPrefix(line, ":count") {
			args := strings.TrimSpace(strings.TrimPrefix(line, ":count"))
			report, err := countCommand(args)
			if err != nil {
				fmt.Println(err)
				continue
			}
		...
		if strings.HasPrefix(line, ":shor") {
			args := strings.TrimSpace(strings.TrimPrefix(line, ":shor"))
			report, err := shorCommand(args)
			if err != nil {
				fmt.Println(err)
				continue
			}
...
func runMeta(line string, ip *Interp) {
	...
	case ":help":
		...
		fmt.Println(":shor N [a=K] [t=K]  Shor period-finding: factor toy N via QPE on a·x mod N")
